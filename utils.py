...
def lose_color(proccessed_obs):
  if proccessed_obs == None: return None
...
def generate_pong_trace(env, start_state, agent, n=200, do_render=True):
  ...
  for i in range(n):
    s_proc  = lose_color(preprocess(all_obs[-2]))
    ss_proc = lose_color(preprocess(all_obs[-1]))

    action = agent.act(((s_proc, ss_proc),all_actions[-1]), show_prob = do_render)
    obs, reward, done, comments = env.step(action)
    if do_render:
      env.render()
    trace.append((((s_proc, ss_proc),all_actions[-1]), action, reward))
    all_obs.append(obs)
...
def get_random_pong_state(env, start_state):
  env.reset()
  env.restore_full_state(start_state)
